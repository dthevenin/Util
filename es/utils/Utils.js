...
if (vsTestStyle) {
  if (vsTestStyle.webkitTransform !== undefined) SUPPORT_3D_TRANSFORM = 'WebKitCSSMatrix' in window && 'm11' in new WebKitCSSMatrix();else if (vsTestStyle.MozTransform !== undefined) SUPPORT_3D_TRANSFORM = 'MozPerspective' in vsTestStyle;else if (vsTestStyle.msTransform !== undefined) SUPPORT_3D_TRANSFORM = 'MSCSSMatrix' in window && 'm11' in new MSCSSMatrix();
 ...
 */
const _requestAnimationFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function (callback) {
  window.setTimeout(callback, 1000 / 60);
};

const requestAnimationFrame = _requestAnimationFrame.bind(window);

const _cancelRequestAnimationFrame = window.cancelRequestAnimationFrame || window.webkitCancelAnimationFrame || window.mozCancelAnimationFrame || window.oCancelAnimationFrame || window.msCancelAnimationFrame || clearTimeout;

const cancelRequestAnimationFrame = _cancelRequestAnimationFrame.bind(window);

const _setImmediate = window.setImmediate || function (func, args) {
  return this.setTimeout(func, 0, args);
};
...
function _defineProperty_api2(obj, prop_name, desc) {
  ...
  if (hasProperty(desc, "enumerable")) d.enumerable = !!desc.enumerable;else d.enumerable = true;
  if (hasProperty(desc, "configurable")) d.configurable = !!desc.configurable;else d.configurable = true;
...
function setElementOpacity(elem, value) {
  ...
  elementStyle.opacity = value === 1 || value === '' ? '' : value < 0.00001 ? 0 : value;
};
...
function safeInnerHTML(elem, html_text) {
  if (!elem || !isString(html_text)) {
    return;
  }

  // MS Window 8 management
  if (window.MSApp && window.MSApp.execUnsafeLocalFunction) window.MSApp.execUnsafeLocalFunction(function () {
...
function setElementWebkitTransform(elem, transform) {
  if (elem && elem.style) elem.style.webkitTransform = transform;else console.warn("setElementTransform, elem null or without style");
}
...
function setElementMSTransform(elem, transform) {
  if (elem && elem.style) elem.style.msTransform = transform;else console.warn("setElementTransform, elem null or without style");
}
...
function setElementMozTransform(elem, transform) {
  if (elem && elem.style) elem.style.MozTransform = transform;else console.warn("setElementTransform, elem null or without style");
}
...
function _setElementTransform(elem, transform) {
  if (elem && elem.style) elem.style.transform = transform;else console.warn("setElementTransform, elem null or without style");
}
...
function getElementMatrixTransform(elem) {
  ...
  if (css.transform) transformMatrix = css.transform;else if (css.webkitTransform) transformMatrix = css.webkitTransform;else if (css.msTransform) transformMatrix = css.msTransform;else if (css.MozTransform) transformMatrix = css.MozTransform;
...
function importFile(path, doc, clb, type, first) {
  ...
  if (type === 'js' || path.search('\\.js') >= 0) {
    ...
    } else doc.head.appendChild(js_effets);
    ...
    } else doc.head.appendChild(css_style);
...
function addCssRule(selector, rule) {
  if (!__app_style_sheet__) {
    ...
    __app_style_sheet__ = document.styleSheets[document.styleSheets.length - 1];
  }
...
const defineProperty = Object.defineProperty ? _defineProperty_api2 : _defineProperty_api1;

const getBoundingClientRect = vsTestElem && vsTestElem.getBoundingClientRect ? _getBoundingClientRect_api2 : _getBoundingClientRect_api1;

/********************************************************************
                         export
*********************************************************************/

export { vsTestElem, vsTestStyle, clearImmediate, setImmediate, requestAnimationFrame, cancelRequestAnimationFrame, CSS_VENDOR, SUPPORT_3D_TRANSFORM, SUPPORT_CSS_TRANSFORM,

// Class functions
extendClass, defineProperty, defineClassProperty, defineClassProperties, clone, free,

// JSON functions
toJSON,

// testing functions
isElement, isArray, isFunction, isString, isNumber, isObject, isUndefined,

// element class
hasClassName, addClassName, removeClassName, toggleClassName,

// string
htmlEncode, strip, camelize, capitalize, underscore, parseJSON,

// element style
addCssRule, addCssRules, getElementHeight, getElementWidth, getElementDimensions, getElementStyle, setElementStyle, setElementOpacity, getElementOpacity, getElementAbsolutePosition, setElementPos, setElementSize, setElementVisibility, isElementVisible, removeAllElementChild, safeInnerHTML, setElementInnerText, setElementTransform, getElementTransform, getElementMatrixTransform, setElementTransformOrigin, getBoundingClientRect,

// other
importFile,
// setActiveStyleSheet,
// preloadTemplate,
__date_reg_exp, _findItem, // export only for testing purpose
